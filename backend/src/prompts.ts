...
export interface MasterPrompt {
  ...
  preservationLock: string;
  negativePrompt: string;
}

/** A child prompt is a single text string injected into the subject map. */
...
export const MASTER_PROMPTS: Record<string, MasterPrompt> = {

  // ── 01  CATALOG IMAGE ────────────────────────────────────────────────────
  CATALOG_IMAGE: {
    systemRole:
      'Act as a master commercial product photographer and high-end jewelry retoucher.',

    cameraSettings:
      'High-end macro product photography, shot on a 100mm macro lens, f/11 aperture for edge-to-edge sharpness, crisp focus stacking, high-resolution editorial print quality.',

    environment:
      'Placed gracefully on a premium, soft cream-colored velvet surface. The surrounding environment is a clean, minimalist, and highly professional luxury studio setting with warm, light neutral tones to beautifully complement and make the jewelry pop.',

    lightingAndPhysics:
      "Soft, diffused Profoto studio lighting with a large overhead softbox. Authentic ambient occlusion. A highly accurate, dark contact shadow anchors the bottom of the ring's band directly into the velvet texture. The metal reflects the softbox light naturally without overexposing.",

    preservationLock:
      'CRITICAL MANDATE: You must strictly preserve the exact original pixels, geometry, facet cuts, true colors, and scale of the masked ring. Do not warp the metal band, do not melt or shift the prongs, and do not alter the internal refractions of the gemstones.',

    negativePrompt:
      '3d render, CGI, digital illustration, plastic texture, artificial AI glow, over-smoothed metal, warped band, distorted facets, melted prongs, floating object, missing contact shadow, unnatural lighting, neon colors, cartoon, painterly effect.',
  },

  // ── 02  JEWELRY ON MODEL ─────────────────────────────────────────────────
  JEWELRY_ON_MODEL: {
    systemRole:
      'Act as a world-class luxury jewelry editorial photographer and fashion art director with credits in Vogue, Cartier, Bulgari, and Van Cleef & Arpels campaigns. You specialize in capturing jewelry on live models in ways that make the piece the absolute star — the model\'s role is to give the jewelry context, scale, and aspirational desirability. Every image must look like a genuine high-production luxury brand campaign photograph — real skin texture, real anatomy, real lighting, never AI-generated or digitally composited. The jewelry must appear organically placed, naturally weighted, and photographically real.',

    cameraSettings:
      'Canon EOS R5 full-frame mirrorless. 85mm f/1.4 portrait lens for natural perspective and beautiful out-of-focus background separation. Aperture f/2.8 — enough depth-of-field to keep the jewelry and immediate skin area sharp while the background softly dissolves. ISO 100–400. 1/250s with TTL strobe. Skin tones rendered accurately at D65 daylight balance. RAW + Lightroom CC grade to Vogue editorial standard.',
  ...
  BACKGROUND_REPLACEMENT: {
    ...
    negativePrompt:
      'Jewelry design altered, gemstone color changed, metal color changed, foreground surface texture changed, jewelry contact shadow removed, jewelry floating above surface, cutline visible around jewelry, halo artifact at background edge, background bleeding through jewelry, background too uniform/flat/digital, color temperature mismatch between jewelry and background, background too busy/distracting from jewelry, text in background, logos, watermarks.',
  },

  // ── 06  LUXURY ENHANCEMENT ───────────────────────────────────────────────
  LUXURY_ENHANCEMENT: {
    systemRole:
      'Act as a senior luxury jewelry retoucher and post-production specialist with credits at Cartier, Tiffany & Co., Harry Winston, and Bulgari. Your specialty is taking good jewelry photographs and transforming them into extraordinary luxury campaign images through meticulous, professional-grade retouching and enhancement. You work with extreme precision and restraint — enhancement should feel invisible, as if the jewelry was always photographed at this level of quality. Enhancement is applied as a post-production layer — the underlying photograph geometry and design do not change.',

    cameraSettings:
      'Preserve the original image\'s camera perspective, composition, and crop exactly. Output at maximum available resolution with no compression artifacts. Enhancement is post-production only.',
  ...
  CUSTOM_PROMPT: {
    ...
    negativePrompt:
      'Jewelry design altered, wrong metal color, wrong gemstone color, jewelry floating, jewelry obscured beyond recognition, amateurish composition, snapshot quality, obvious AI artifacts (melted text, impossible anatomy, fused elements), low resolution output, missing contact shadow, physically impossible scene elements, dark or muddy overall exposure that hides jewelry detail.',
  },

  // BULK_GENERATION inherits from CATALOG_IMAGE
  BULK_GENERATION: {
    systemRole:
      'Act as a world-class commercial jewelry photographer with over 20 years of high-end editorial experience. You specialize in pristine macro catalog photography for luxury jewelry brands. Your mission is to produce a photograph that looks physically real, hyper-detailed, and commercially polished — not rendered or AI-generated. Every image must feel like it was captured inside a professional luxury product studio under controlled lighting.',

    cameraSettings:
      'Canon EOS R5 full-frame mirrorless, 100mm f/2.8L IS USM Macro lens. Aperture f/11 for total edge-to-edge sharpness and maximum depth of field. ISO 100 for zero grain. 1/125s shutter speed with studio strobe synchronization. Tripod-locked for absolute stability. RAW capture, color-calibrated to D65 neutral daylight.',

    environment:
      'The jewelry piece rests on a premium matte cream-white velvet surface with an ultra-fine micro-texture. The surface curves gently upward in the background (infinity sweep), seamlessly transitioning to a soft warm-white studio backdrop. The surrounding environment is clean, minimalist, and impeccably professional — no objects, no distracting textures, no noise. The ambient color temperature is 5500K warm-neutral.',

    lightingAndPhysics:
      'Primary light: a large 120cm Profoto octabank softbox positioned overhead-front at a 45-degree angle, producing soft, wrapping light that gently fills the metal\'s contours. Fill light: a large reflector panel placed opposite the key light. All shadows are soft, natural, and physically accurate — especially the contact shadow where the jewelry touches the velvet surface. Metal reflections are realistic. Gemstones exhibit authentic internal refractions.',

    preservationLock:
      'ABSOLUTE MANDATE — Preserve the exact physical integrity of the jewelry piece: no geometry changes, no prong alteration, no gemstone color change, no band warping, no artificial smoothing, correct scale, piece physically grounded — never floating.',
...
export const CHILD_PROMPTS: Record<string, ChildPrompt> = {
...
export const WORKFLOW_INTENTS: Record<string, string> = {
...
export const UNIVERSAL_NEGATIVE_PROMPT =
  'low quality, blurry, distorted jewelry, melted metal, warped ring, asymmetrical gemstones, broken prongs, extra gemstones, floating jewelry, missing contact shadow, deformed chain, unrealistic reflections, poor lighting, cartoon, CGI look, 3D render, digital illustration, AI artifacts, text overlay, watermark, over-stylization, neon colors, plastic texture.';

// ─────────────────────────────────────────────────────────────────────────────
// HELPER — map a raw jewelry type string to a child prompt key
// ─────────────────────────────────────────────────────────────────────────────

export function getChildPromptKey(rawType: string, workflow?: string): string {
  const t = rawType.toLowerCase().trim();

  // Workflow-specific special keys
  if (workflow === 'GEMSTONE_COLOR_CHANGE') {
    if (t.includes('ruby')) return 'gemstone_ruby';
    if (t.includes('sapphire')) return 'gemstone_sapphire';
    if (t.includes('emerald')) return 'gemstone_emerald';
    if (t.includes('diamond')) return 'gemstone_diamond';
    if (t.includes('moissanite')) return 'gemstone_moissanite';
  }

  if (workflow === 'LUXURY_ENHANCEMENT') {
    if (t.includes('rose gold')) return 'enhancement_rose_gold';
    if (t.includes('white gold') || t.includes('platinum')) return 'enhancement_white_gold_platinum';
    if (t.includes('yellow gold') || t.includes('gold')) return 'enhancement_yellow_gold';
  }

  if (workflow === 'BACKGROUND_REPLACEMENT') {
    if (t.includes('white') || t.includes('seamless')) return 'background_white_seamless';
    if (t.includes('black') || t.includes('acrylic')) return 'background_black_acrylic';
    if (t.includes('marble')) return 'background_marble';
    if (t.includes('velvet')) return 'background_dark_velvet';
    if (t.includes('golden') || t.includes('sunset') || t.includes('outdoor')) return 'background_golden_hour';
    if (t.includes('interior') || t.includes('lifestyle') || t.includes('luxury')) return 'background_luxury_interior';
    if (t.includes('transparent') || t.includes('png') || t.includes('cutout')) return 'background_transparent';
  }

  // Standard jewelry type routing
  if (t.includes('stud')) return 'earrings_studs';
  if (t.includes('drop')) return 'earrings_drops';
  if (t.includes('hoop')) return 'earrings_hoops';
  if (t.includes('earring')) return 'earrings_studs';
  if (t.includes('multiple') || t.includes('set') || t.includes('multi')) return 'multiple_items';
  ...
  if (t.includes('brooch')) return 'brooch';
  if (t.includes('anklet')) return 'anklet';
  if (t.includes('cufflink')) return 'cufflinks';
