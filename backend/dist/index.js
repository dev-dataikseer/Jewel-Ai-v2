...
function resolveModelAlias(alias) {
    if (!alias)
        return MODEL_ALIAS_MAP['model_2'];
...
app.use((req, res, next) => {
    ...
    res.on('finish', () => {
        logger_1.logger.request(req.method, req.originalUrl, res.statusCode, Date.now() - start, req.body && Object.keys(req.body).length ? { keys: Object.keys(req.body) } : undefined);
    });
    next();
});
...
async function seedDefaults() {
    ...
    for (const wfId of workflowIds) {
        const lib = prompts_1.MASTER_PROMPTS[wfId];
        if (!lib)
            continue;
        await prisma_1.default.promptTemplate.upsert({
            where: { workflow: wfId },
            update: {
    ...
    for (const s of subjectSeeds) {
        const lib = prompts_1.CHILD_PROMPTS[s.key];
        if (!lib)
            continue;
        await prisma_1.default.subjectPrompt.upsert({
    ...
    await prisma_1.default.providerSetting.upsert({
...
function resolveAssetPaths(body) {
    return {
        ...body,
...
app.get('/api/diagnose', async (_req, res) => {
    const diagnosticResults = {
        ...
        database: { status: 'unknown', path: path_1.default.join(__dirname, '../prisma/dev.db') },
    ...
    try {
        ...
        const testFile = path_1.default.join(uploadsDir, '.diagnose-test');
    ...
    try {
        ...
        const assetCount = await prisma_1.default.asset.count().catch((err) => {
            throw new Error(`Asset table error: ${err.message}`);
        });
        const jobCount = await prisma_1.default.job.count().catch((err) => {
...
app.get('/api/config/options', (_req, res) => {
    res.json({
        ...
        workflows: WORKFLOWS,
        workflowLabels: queue_1.WORKFLOW_LABELS,
        geminiModels: [
...
app.post('/api/assets/bulk-upload', upload.array('files', 30), async (req, res) => {
    const files = req.files;
    if (!files || files.length === 0) {
        res.status(400).json({ error: 'No files uploaded' });
        return;
    }
    const assets = await Promise.all(files.map((file) => prisma_1.default.asset.create({
...
app.post('/api/jobs', async (req, res) => {
    try {
        ...
        const { projectName, aspectRatio, personGeneration, numberOfImages, modelAlias, stylePresetId, metalType, gemstoneType, gemstoneCut, gemstoneTargetColor, backgroundStyle, lightingStyle, ...jobData } = body;
        ...
        const providerMetadataObj = {};
        if (aspectRatio)
            providerMetadataObj.aspectRatio = aspectRatio;
        if (personGeneration)
            providerMetadataObj.personGeneration = personGeneration;
        if (numberOfImages)
            providerMetadataObj.numberOfImages = numberOfImages;
        ...
        await (0, queue_1.enqueueImageJob)(job.id);
        res.json({ job });
    }
    catch (error) {
        logger_1.logger.error('Job creation error', { error: error.message });
...
app.post('/api/jobs/bulk', async (req, res) => {
    try {
        ...
        const assets = await prisma_1.default.asset.findMany({ where: { id: { in: assetIds } } });
        ...
        logger_1.logger.error('Bulk job error', { error: error.message });
...
app.post('/api/jobs/:id/regenerate', async (req, res) => {
    try {
        const source = await prisma_1.default.job.findUnique({ where: { id: req.params.id } });
        if (!source) {
        ...
        if (req.body.aspectRatio || req.body.personGeneration || req.body.numberOfImages) {
            ...
            if (req.body.aspectRatio)
                sourceMeta.aspectRatio = req.body.aspectRatio;
            if (req.body.personGeneration)
                sourceMeta.personGeneration = req.body.personGeneration;
            if (req.body.numberOfImages)
                sourceMeta.numberOfImages = req.body.numberOfImages;
        ...
        logger_1.logger.error('Regeneration error', { error: error.message });
...
app.get('/api/style-presets', async (req, res) => {
    ...
    const presets = await prisma_1.default.stylePreset.findMany({
        where: {
...
app.post('/api/style-presets', async (req, res) => {
    const preset = await prisma_1.default.stylePreset.create({
        data: {
...
app.delete('/api/style-presets/:id', async (req, res) => {
    await prisma_1.default.stylePreset.delete({ where: { id: req.params.id } });
    res.json({ success: true });
});
app.get('/api/prompt-templates', async (_req, res) => {
    const prompts = await prisma_1.default.promptTemplate.findMany({
        orderBy: { updatedAt: 'desc' },
    });
    res.json(prompts);
});
app.post('/api/prompt-templates', async (req, res) => {
    const prompt = await prisma_1.default.promptTemplate.upsert({
...
app.get('/api/subject-prompts', async (_req, res) => {
    const prompts = await prisma_1.default.subjectPrompt.findMany({
        orderBy: { jewelryType: 'asc' },
    });
    res.json(prompts);
});
app.post('/api/subject-prompts', async (req, res) => {
    const prompt = await prisma_1.default.subjectPrompt.upsert({
...
app.delete('/api/subject-prompts/:id', async (req, res) => {
    await prisma_1.default.subjectPrompt.delete({ where: { id: req.params.id } });
    res.json({ success: true });
});
app.post('/api/prompt-templates/test', async (req, res) => {
    const prompt = await (0, queue_1.composePrompt)({
        ...req.body,
    });
    res.json({ prompt });
});
app.get('/api/provider-settings', async (_req, res) => {
    const settings = await prisma_1.default.providerSetting.findMany();
...
app.post('/api/provider-settings/gemini', async (req, res) => {
    ...
    const settings = await prisma_1.default.providerSetting.upsert({
        where: { provider: 'GEMINI' },
        update: {
            modelName,
...
app.post('/api/provider-settings/gemini/test', async (req, res) => {
    ...
    try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${modelName || 'gemini-1.5-flash'}:generateContent`, {
            method: 'POST',
...
app.get('/api/admin/metrics', async (_req, res) => {
    const [totalJobs, completedJobs, failedJobs, totalAssets, totalBatches, favorites] = await Promise.all([
        ...
        prisma_1.default.job.count({ where: { status: 'COMPLETED' } }),
        prisma_1.default.job.count({ where: { status: 'FAILED' } }),
        prisma_1.default.asset.count(),
        prisma_1.default.batch.count(),
        prisma_1.default.favorite.count(),
    ]);
    const recentFailures = await prisma_1.default.job.findMany({
        where: { status: 'FAILED' },
        take: 5,
        orderBy: { createdAt: 'desc' },
...
app.get('/api/rates/live', async (_req, res) => {
    try {
        ...
        const quotes = await Promise.all(symbols.map((sym) => yahooFinance.quote(sym).catch((err) => {
            logger_1.logger.error(`Error fetching quote for ${sym}`, { error: err?.message });
            return null;
        })));
        ...
        const formatMetalData = (quote, name) => {
            if (!quote)
                return null;
...
app.delete('/api/rates/:id', async (req, res) => {
    await prisma_1.default.rateEntry.delete({ where: { id: req.params.id } });
    res.json({ success: true });
});
app.post('/api/favorites/:jobId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Favorite creation error', { error: error.message });
...
app.get('/api/logs/:type', (req, res) => {
    ...
    const logs = logger_1.logger.getLogs(type, lines);
...
if (process.env.NODE_ENV === 'production') {
    const frontendBuildPath = path_1.default.join(__dirname, '../../frontend/out');
    app.use(express_1.default.static(frontendBuildPath, { extensions: ['html'] }));
    app.get(/.*/, (req, res) => {
...
seedDefaults().catch((error) => {
    logger_1.logger.error('Failed to seed defaults', { error: error.message });
});
